...
class BertModel(object):
  ...
  def __init__(self,
    ...
    if entity_type_ids is None:
      entuty_type_ids = tf.zeros(shape=[batch_size, seq_length], dtype=tf.int32)
    ...
    with tf.variable_scope(scope, default_name="bert"):
      with tf.variable_scope("embeddings"):
        ...
        self.embedding_output = embedding_postprocessor(
            ...
            entity_type_embedding_name="entity_type_embeddings",
...
def embedding_postprocessor(input_tensor,
  ...
  if use_token_type:
    ...
    entity_type_table = tf.get_variable(
        name=entity_type_embedding_name,
        shape=[entity_type_vocab_size, width],
        initializer=create_initializer(initializer_range))
    # This vocab will be small so we always do one-hot here, since it is always
    # faster for a small vocabulary.
    flat_entity_type_ids = tf.reshape(entity_type_ids, [-1])
    one_hot_ids = tf.one_hot(flat_entity_type_ids, depth=entity_type_vocab_size)
